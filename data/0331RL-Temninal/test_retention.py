...
def create_test_data(users_data):
    ...
    包含用户 ID、登录日期、行为类型三个字段
    
    参数:
        users_data: dict {user_id: [login_dates]}
    """
    data = []
    action_types = ['login', 'view', 'click', 'purchase']
...
def test_single_user_no_retention():
...
def test_single_user_with_retention():
...
def test_multiple_users_mixed_retention():
    """测试多个用户混合情况"""
    users_data = {
        1: ['2024-01-01', '2024-01-08'],
...
def test_duplicate_dates_same_user():
...
def test_non_consecutive_dates():
...
def test_user_logs_in_before_day_n():
...
def test_user_logs_in_after_day_n():
...
def test_users_with_different_cohort_dates():
...
def test_n_day_retention_3_day():
    """测试 3 日留存率计算"""
    users_data = {
...
def test_dataframe_includes_action_type():
    """验证 DataFrame 中包含行为类型字段，且不影响计算结果"""
    users_data = {
        1: ['2024-01-01', '2024-01-08'],
        2: ['2024-01-01']
    }
    ...
    assert 'action_type' in df.columns, "DataFrame 应该包含 action_type 字段"
    assert df['action_type'].nunique() > 1, "应该包含多种行为类型"
    assert calculate_n_day_retention(df, n_days=7) == 0.5
