...
class TestCalculateNDayRetention(unittest.TestCase):
    """测试 calculate_n_day_retention 函数"""

    def test_empty_dataframe(self):
        """测试空 DataFrame 返回 0"""
        df = pd.DataFrame(columns=['user_id', 'login_date', 'action_type'])
        self.assertEqual(calculate_n_day_retention(df, mode='all_cohorts_mixed'), 0.0)
    ...
    def test_single_user_no_retention(self):
        ...
        self.assertEqual(calculate_n_day_retention(df, n_days=7, mode='all_cohorts_mixed'), 0.0)
    ...
    def test_single_user_with_retention(self):
        ...
        df = create_test_data(users_data)
        self.assertEqual(calculate_n_day_retention(df, n_days=7, mode='all_cohorts_mixed'), 1.0)
    ...
    def test_multiple_users_mixed_retention(self):
        ...
        df = create_test_data(users_data)
        self.assertEqual(calculate_n_day_retention(df, n_days=7, mode='all_cohorts_mixed'), 0.5)
    ...
    def test_n_day_retention_3_day(self):
        ...
        df = create_test_data(users_data)
        self.assertAlmostEqual(calculate_n_day_retention(df, n_days=3, mode='all_cohorts_mixed'), 0.666666666, places=6)
